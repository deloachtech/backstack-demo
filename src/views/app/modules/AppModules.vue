<template>
  ...
  <div v-else>
    <div v-if="data.length === 0" class="alert alert-info">There are no optional modules are available at this time.
      Contact support if you'd like one created for your specific requirement.</div>
    ...
    <div v-else class="bva-optional-features mb-5">
      <div :class="[data.length === 0 ? 'row-cols-1' : 'row-cols-1 row-cols-lg-3', 'row g-5']">
        <div v-for="record in data" :key="record.feature_id" class="col d-flex align-items-start">
          <div
            :class="['icon-square text-body-emphasis bg-body-secondary d-inline-flex align-items-center justify-content-center fs-4 flex-shrink-0 me-3', { 'semi-transparent': activateModuleId === record.id }]">
            <!-- 
            The demo project stores a bootstrap icon (e.g. <i class="bi bi-exclamation-circle"></i>) string as the feature.image. 
            -->
          ...
          <div class="position-relative">
            <div :class="{ 'semi-transparent': activateModuleId === record.id }">
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown">
                  <button class="btn btn-sm" type="button" :id="record.id" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    <i class="bi bi-gear fs-4"></i>
                  </button>
                  <ul class="dropdown-menu dropdown-menu-end" :aria-labelledby="record.id">
                    <li><a class="dropdown-item" href="javascript:void(0)"
                        @click="handleOptionClick({ key: 'more-info', record: record })">More info</a></li>
                    <li><a class="dropdown-item" href="javascript:void(0)"
                        @click="handleOptionClick({ key: 'settings', record: record })">Settings</a></li>
                    <li>
                      <hr class="dropdown-divider" />
                    </li>
                    <li v-if="record.active"><a class="dropdown-item" href="javascript:void(0)"
                        @click="handleOptionClick({ key: 'deactivate', record: record })">Deactivate</a></li>
                    <li v-else><a class="dropdown-item" href="javascript:void(0)"
                        @click="handleOptionClick({ key: 'activate', record: record })">Activate</a></li>
                  </ul>
                </div>
              </div>
  ...
  <Modal :open="modal" @cancel="modal = false" @submit="" :submitting="submitting" cancel-label="Close"
    :submit-label="modalSubmitLabel" :heading="(record.image ? record.image + ' &nbsp;' : '') + record.title">
